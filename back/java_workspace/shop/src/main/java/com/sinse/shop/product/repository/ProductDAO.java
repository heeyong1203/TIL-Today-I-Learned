...
public class ProductDAO {
	...
	public List selectRecentList(int limit) {
		...
		ResultSet rs3 = null;
		PreparedStatement pstmt4 = null; // image
		ResultSet rs4 = null;
		...
		sql.append(" order by product_id desc limit ?"); // 이 와중에 이미지, 사이즈, 색상을 서브쿼리로 추가하여야 함...
		...
		try {
			...
			while (rs.next()) { // 상품의 수 만큼...
				...
				sql.delete(0, sql.length()); // stringBuffer()의 문자열 비우기
				...
				while (rs2.next()) { // 사이즈들을 product model의 size list에 담자...
					Size size = new Size();
				...
				sql.delete(0, sql.length());
				sql.append("select filename from");
				sql.append(" product_img where product_id = ?");
				...
				subCategory.setSub_name(rs.getString("sub_name"));
				product.setSubCategory(subCategory); // 서브 카테고리 모델을 Product 모델에 대입
